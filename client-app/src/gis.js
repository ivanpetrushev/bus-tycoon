export let gis = {
    ...
    createCoordLatLon: function (coord, bearing, distance) {
        /** http://www.movable-type.co.uk/scripts/latlong.html
         * φ is latitude, λ is longitude,
         * θ is the bearing (clockwise from north),
         * δ is the angular distance d/R;
        ...
        let
            ...
            δ = Number(distance) / radius, // angular distance in radians
            θ = gis.toRad(Number(bearing));
        let φ1 = gis.toRad(coord[0]),
            λ1 = gis.toRad(coord[1]);
        ...
        return [gis.toDeg(φ2), gis.toDeg(λ2)];
    },
    /**
     * All coordinates expected EPSG:4326
     * @param {Array} start Expected [lon, lat]
    ...
    getBearingLatLon: function (start, end) {
        let
